    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex flex-col md:grid grid-cols-3 md:flex-row bg-white px-5 py-5 rounded-xl my-3 gap-5">
                ...
                <SelectUi items={Array.isArray(items_data) ? items_data : []} title='Военный конфликт' item={null} func={(item: {title: string, id: number}) => {if (item) {setValue('armed_conflict', item.id), {required: true}}}} description={errors.armed_conflict && <span className="text-red-500">Обязательно к заполнению</span>}/>
            </div>
