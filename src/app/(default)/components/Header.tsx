...
export default function Header() {
  ...
  const logout = () => {
    ...
    return <footer>
    {is_auth && <div className="bg-indigo-600 p-1.5 text-center font-bold text-white">Вы авторизованы как сотрудник <button onClick={logout} className="ml-2 bg-white text-black hover:text-white hover:bg-black px-2 rounded-md">Выйти</button></div>}
    ...
    <div className="gap-3 hidden md:flex">
      {is_auth && <>
        <Link href="/person/create" className="hover:text-black/60 transition duration-200 ease-in-out">Добавить бойца</Link>
        <Link href="/panel/profile" className="hover:text-black/60 transition duration-200 ease-in-out">Профиль</Link>
      </>}
      <Link href="/filter" className="hover:text-black/60 transition duration-200 ease-in-out">Поиск</Link>
